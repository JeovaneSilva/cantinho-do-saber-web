...
export default function Materiais() {
  const { materiais, isLoading, refreshMateriais, deleteMaterial } =
    useMateriais();
  ...
  const [selectedAluno, setSelectedAluno] = useState("todos");

  useEffect(() => {
    materiaService
      .findAll()
      .then((data) => {
        setSubjectOptions(["Todas as Matérias", ...data.map((m) => m.nome)]);
      })
      .catch(console.error);
    ...
    alunosService
      ...
      .then((data) => {
        ...
        setAlunoOptions([
          { id: "todos", nome: "Todos os Alunos" },
          { id: "geral", nome: "Materiais Gerais (Sem Aluno)" },
          ...alunosMapeados,
        ]);
      })
      .catch(console.error);
  ...
  const filteredMaterials = materiais.filter((material) => {
    const matchesSearch = material.titulo
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesSubject =
      selectedSubject === "Todas as Matérias" ||
      material.materia?.nome === selectedSubject;

    let matchesAluno = true;
    if (selectedAluno === "geral") {
      matchesAluno = !material.alunoId;
    } else if (selectedAluno !== "todos") {
      matchesAluno = String(material.alunoId) === selectedAluno;
    }

    return matchesSearch && matchesSubject && matchesAluno;
  });
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 animate-scale-in">
          ...
          <div className="card-educational text-center">
            ...
            <p className="text-2xl font-bold text-foreground">{stats.imagem}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <select
            ...
            className="px-4 py-2.5 rounded-lg border border-border bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer hover:bg-muted/50 max-w-[200px]"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredMaterials.map((material) => {
                ...
                return (
                  <div
                    key={material.id}
                    className="card-educational group relative hover:border-primary/50 transition-all"
                  >
                    <button
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-4">
                      ...
                      {material.aluno?.nome && (
                        <span className="px-2 py-1 text-xs font-medium rounded-md bg-secondary/10 text-secondary flex items-center gap-1">
                          <User className="w-3 h-3" />{" "}
                          {material.aluno.nome.split(" ")[0]}
                        </span>
                      )}
                    </div>
