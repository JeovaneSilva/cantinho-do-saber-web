          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative w-full sm:w-auto">
               ...
               <select 
                 ...
                 className="w-full sm:w-auto pl-9 pr-8 py-2.5 rounded-lg border border-border bg-card text-foreground font-medium appearance-none cursor-pointer hover:bg-muted focus:ring-2 focus:ring-primary/20 outline-none"
               >
            ...
            <button 
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg gradient-primary text-primary-foreground font-semibold shadow-lg hover:opacity-90"
            >
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-scale-in">
              <div className="card-educational flex items-center gap-4">
                <div className="w-12 h-12 shrink-0 rounded-xl bg-success/15 flex items-center justify-center"><TrendingUp className="w-6 h-6 text-success" /></div>
                <div className="min-w-0">
                  <p className="text-sm text-muted-foreground truncate">Recebido</p>
                  <p className="text-xl lg:text-2xl font-bold text-success truncate">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalReceived)}</p>
                </div>
              </div>
              <div className="card-educational flex items-center gap-4">
                <div className="w-12 h-12 shrink-0 rounded-xl bg-warning/15 flex items-center justify-center"><Clock className="w-6 h-6 text-warning" /></div>
                <div className="min-w-0">
                  <p className="text-sm text-muted-foreground truncate">Pendente</p>
                  <p className="text-xl lg:text-2xl font-bold text-warning truncate">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalPending)}</p>
                </div>
              </div>
              <div className="card-educational flex items-center gap-4">
                <div className="w-12 h-12 shrink-0 rounded-xl bg-destructive/15 flex items-center justify-center"><TrendingDown className="w-6 h-6 text-destructive" /></div>
                <div className="min-w-0">
                  <p className="text-sm text-muted-foreground truncate">Atrasado</p>
                  <p className="text-xl lg:text-2xl font-bold text-destructive truncate">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalOverdue)}</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 xl:pb-0 scrollbar-none snap-x">
                {(['ALL', 'PAGO', 'PENDENTE', 'ATRASADO'] as const).map((status) => (
                  <button
                    ...
                    className={cn(
                      "px-4 py-2 text-sm sm:text-base sm:py-2.5 rounded-lg font-medium transition-all whitespace-nowrap snap-start shrink-0",
                      filterStatus === status
                ...
                <table className="w-full min-w-[700px]"> 
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      <th className="text-left py-4 px-6 font-semibold text-foreground whitespace-nowrap">Aluno</th>
                      <th className="text-left py-4 px-6 font-semibold text-foreground whitespace-nowrap">Mês</th>
                      <th className="text-left py-4 px-6 font-semibold text-foreground whitespace-nowrap">Vencimento</th>
                      <th className="text-left py-4 px-6 font-semibold text-foreground whitespace-nowrap">Valor</th>
                      <th className="text-left py-4 px-6 font-semibold text-foreground whitespace-nowrap">Status</th>
                      <th className="text-center py-4 px-6 font-semibold text-foreground whitespace-nowrap">Ações</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredPayments.map((payment) => {
                      ...
                      return (
                        <tr key={payment.id} className="border-b border-border hover:bg-muted/30 transition-colors group">
                          ...
                          <td className="py-4 px-6 text-muted-foreground whitespace-nowrap">{payment.mesReferencia}</td>
